...
const PageOrder: React.FC = () => {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useOrders(currentStatus, requestParams);
  ...
  const tableColumns = useMemo<TableProps<OrderData>["columns"]>(
      ...
      },
    {
      ...
      },
      {
        ...
        dataIndex: "createdAt",
        key: "createdAt",
        render: (date: string) => ConvertUtil.convertVietNamDateTime(date),
      },
      {
        sorter: true,
        title: "Trạng thái",
      ...
      },
      {
        ...
        key: "tt",
        render: (r: OrderData) => (
          <Flex>{ConvertUtil.getOrderThanhToanLabel(r)}</Flex>
        ),
      },
      {
        title: "Thao tác",
          ...
          <Flex gap={10}>
            ...
            <Flex flex={1} vertical gap={20}>
              <BTNReload
                loading={isFetching}
                onClick={() => refetch()}
                toolTipTitle="Tải lại trạng thái"
              />
              <Table
                size="small"
                rowKey="uuid"
