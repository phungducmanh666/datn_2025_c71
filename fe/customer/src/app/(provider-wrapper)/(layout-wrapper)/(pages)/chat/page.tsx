                        ...
                        >
                            <Flex gap={10}>
                                {products.map((product) => (
                                    <div key={product.uuid} style={{width: "100%"}}>
                                        <ProductCard data={product} size="small" />
                                    </div>
                                ))}
                            </Flex>
                        </div>
...
const ChatPage: React.FC<ChatPageProps> = ({ }) => {
    ...
    useEffect(() => {
        const staff = LocalStorageUtil.getCustomer();
        if (!!!staff) {
    ...
    useEffect(() => {
        setMessage(LocalStorageUtil.getMessages() || []);
    }, []);
    ...
    useEffect(() => {
        ...
        LocalStorageUtil.setMessages(message);
    ...
    // 1. Tạo message mới của user
    const userMessage: Message = { role: "user", content: trimmedMessage };

    // 2. Cập nhật UI trước (hiển thị ngay tin nhắn của user)
    setMessage(prevMessages => [...prevMessages, userMessage]);

    // 3. Lấy 3 tin nhắn gần nhất (trước tin nhắn hiện tại)
    const recentMessages = message.slice(-3);

    // 4. Gửi request đến server kèm ngữ cảnh hội thoại
    chatMutate({
        userId: userId,
        message: trimmedMessage,
        history: [...recentMessages, userMessage] // giữ đúng thứ tự hội thoại
    });

    // 5. Xóa nội dung ô input
    setCurrentMessage("");
    };
