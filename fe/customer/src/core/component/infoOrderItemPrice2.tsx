...
interface OrderItemPriceInfo2Props {
    product: OrderProduct;
    onGetPriceInfo?: (data: DiscountInfo) => any
}

const OrderItemPriceInfo2: React.FC<OrderItemPriceInfo2Props> = ({ product, onGetPriceInfo }) => {
                ...
                <div className="flex flex-col">
                    <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-xl font-bold text-red-600">
                            {ConvertUtil.formatVNCurrency(priceInfo.finalPrice * product.number)}
                        </span>
                        {/* Giá cũ: Xám, Nhỏ, Gạch ngang */}
                        <span className="text-xs text-gray-400 line-through decoration-gray-400">
                            {ConvertUtil.formatVNCurrency(product.price * product.number)}
                        </span>
                    </div>
                    ...
                    <div className="flex items-center mt-1">
                        <span className="text-[10px] font-semibold text-red-600 bg-red-50 px-2 py-0.5 rounded border border-red-100">
                            Tiết kiệm {ConvertUtil.formatVNCurrency(priceInfo.savedAmount * product.number)}
                        </span>
                    </div>
                </div>
            ) : (
...
export default OrderItemPriceInfo2;