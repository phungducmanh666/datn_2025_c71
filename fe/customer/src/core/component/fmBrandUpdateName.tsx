...
import {
  useCheckBrandName,
  useUpdateBrandName,
} from "@hook/productHook/brandHook";
...
interface FMBrandUpdateNameProps {
  brand: BrandData;
  open: boolean;
  onUpdated?: () => any;
  onCancle?: () => any;
...
const FMBrandUpdateName: React.FC<FMBrandUpdateNameProps> = ({
  brand,
  open,
  onUpdated,
  onCancle,
  ...
  const debouncedCheck = debounce((value: string) => {
    ...
    if (name != brand.name) checkName(name);
  }, 500);
  ...
  const handleUpdate = () => {
    form.validateFields().then((values) => {
      ...
      updateMutate({ uuid: brand.uuid, name });
    });
  };
...
export default FMBrandUpdateName;
