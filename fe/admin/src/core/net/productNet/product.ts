...
export class ProductAPI {
  ...
  static async getProductJson(uuid: string): Promise<string> {
    try {
      const res = await fetchWithAuth(`${this.prefix}/${uuid}/json`, {
        method: "GET",
      });
  ...
  //#region statistics
  static async getProductStatusStatistics(): Promise<StatusStatisticsData[]> {
    try {
      ...
      const items = text
        .split("\n")
        .filter(Boolean)
        .map((line) => JSON.parse(line) as StatusStatisticsData);
