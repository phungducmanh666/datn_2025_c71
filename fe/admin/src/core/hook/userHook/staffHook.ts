...
import { StaffAPI } from "@net/userNet/staff";
...
export const useStaffAccount = (code: string) => {
  return useQuery({
    queryKey: ["staffAccount", code],
    queryFn: () => StaffAPI.getAccount(code),
    staleTime: 200,
    enabled: !!code,
  });
};
...
export const useStaffCountStatistics = () => {
  return useQuery({
    queryKey: ["useStaffCountStatistics"],
    queryFn: () => StaffAPI.getCountStatistics(),
    staleTime: 200,
  });
};
