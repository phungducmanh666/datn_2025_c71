...
export const useProductJson = (uuid: string) => {
  return useQuery({
    queryKey: ["productJson", uuid],
    queryFn: () => ProductAPI.getProductJson(uuid),
    staleTime: 200,
  });
};
...
export const useProducts = (filter?: ProductQueryDataFilter) => {
  return useQuery({
    ...
    staleTime: 200, // có thể tùy chỉnh nếu muốn cache lâu hơn
  });
};
...
//#region statistics
export const useProductStatusStatistics = () => {
  return useQuery({
    queryKey: ["useProductStatusStatistics"],
    queryFn: () => ProductAPI.getProductStatusStatistics(),
    staleTime: 200,
  });
};
//#endregion
