...
import { Collapse, Flex, MenuProps, Spin, Typography } from "antd";
...
const PurchaseReceiptDetail: React.FC<PurchaseReceiptDetailProps> = ({
  ...
  const header = useMemo(() => {
    const formattedDate = ConvertUtil.convertVietNamDate(receipt.receiptDate);
  ...
  return (
    <>
      <Flex vertical gap={40}>
        ...
        <Flex>{!!!data?.receipt && <BTNDropDown items={menuItems} />}</Flex>
      </Flex>
    </>
  );
};
